...
class Vector:
    ...
    def Heading(self, toDegree=False):
        theta = atan2(self.y, self.x)       # returns angle between coordinate and positive x axis
        if toDegree:
            return theta * (180 / pi)
        return theta
    ...
    def WithinRange(a, b, dist):
        # ants have to enter the nest before going out again
        #
        if abs(a.x - b.x) > dist:
            return False
        if abs(a.y - b.y) > dist:
    ...
    def Scale(self, l):
        # ants don't get out of nest without the else part
        # I think it adds some randomness to motion from A to B
        mag = self.Magnitude()
        if mag == 0:
            return Vector()
        else:
